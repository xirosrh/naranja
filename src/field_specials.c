    ...
    for (i = 0; i < gSpecialVar_0x8004; i++)
    {
        if (myLinkPlayerNumber != i)
        {
            ...
            case VERSION_EMERALD:
                if (gLinkPlayers[i].gender == 0)
                    linkSpriteId = OBJ_EVENT_GFX_RIVAL_ASH_NORMAL;
                else
                    linkSpriteId = OBJ_EVENT_GFX_RIVAL_MISTY_NORMAL;
                break;
            default:
                if (gLinkPlayers[i].gender == 0)
                    linkSpriteId = OBJ_EVENT_GFX_RIVAL_ASH_NORMAL;
...
static void LoadLinkPartnerObjectEventSpritePalette(u16 graphicsId, u8 localEventId, u8 paletteNum)
{
    ...
    if (graphicsId == OBJ_EVENT_GFX_LINK_RS_BRENDAN ||
        ...
        graphicsId == OBJ_EVENT_GFX_RIVAL_ASH_NORMAL ||
        graphicsId == OBJ_EVENT_GFX_RIVAL_MISTY_NORMAL)
    {
        ...
        if (obj != OBJECT_EVENTS_COUNT)
        {
            ...
            case OBJ_EVENT_GFX_LINK_RS_MAY:
                ...
                break;
            case OBJ_EVENT_GFX_RIVAL_ASH_NORMAL:
                LoadPalette(gObjectEventPal_Ash, OBJ_PLTT_ID(adjustedPaletteNum), PLTT_SIZE_4BPP);
                break;
            case OBJ_EVENT_GFX_RIVAL_MISTY_NORMAL:
                LoadPalette(gObjectEventPal_Misty, OBJ_PLTT_ID(adjustedPaletteNum), PLTT_SIZE_4BPP);
                break;
            }
        }
    }
...
void Naranja_CheckPartyMove(void)
{
    enum Move move = gSpecialVar_0x8004;
    ...
    for (u32 partyIndex = 0; partyIndex < CalculatePlayerPartyCount(); partyIndex++)
    {
        ...
        if (!GetMonData(mon, MON_DATA_IS_EGG) && MonKnowsMove(mon, move))
        {
            ...
            gSpecialVar_0x8004 = GetMonData(mon, MON_DATA_SPECIES);
            break;
        }
    }
}
